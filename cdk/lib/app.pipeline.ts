...
export class AppPipeline extends Stack {
    constructor(scope: Construct, id: string, props: StackProps) {
        ...
        const shell = new ShellStep('ShellStep', {
            ...
            installCommands: ['cd cdk', 'npm install', 'cd ..', 'cd app', 'npm install', 'cd ..'],
            commands: ['cd cdk', 'npm run cdk synth -- --output ../cdk.out', 'cd ..', 'cd app', 'npm run build'],
        })
        ...
        stage.addAction(
            new CodeDeployServerDeployAction({
                ...
                role: new Role(this.scope, 'PrivateUniverseCodeDeployRole', {
                    ...
                    managedPolicies: [ManagedPolicy.fromAwsManagedPolicyName('AWSCodeDeployDeployerAccess')],
                }),
                input: new Artifact('ShellStep_app'),
                deploymentGroup: ServerDeploymentGroup.fromServerDeploymentGroupAttributes(
                    this.scope,
